...
//    ros::Publisher  pub     =   nh.advertise<cpg_driver_msg::cpg_param_change>("module_change1",10);//simulation
    ros::Publisher  pub     =   nh.advertise<cpg_driver_msg::cpg_param_change>("module_change",10);//real robots
    ...
    param.freq = 1.45;
    param.alfa_pec_l=param.alfa_pec_r=1.5;
    param.A_ca=param.X_ca=0;
    ...
    while(ros::ok())
    {
        ...
        u       = abs(u)<0.8 ? u : (u>0 ? 0.8:-0.8);
        param.A_pec_l=param.A_pec_r=u;
        //发布
        pub.publish(param);
        //解锁
